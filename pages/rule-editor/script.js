...
document.addEventListener('DOMContentLoaded', function() {
    const zdhf_tjgz = document.getElementById('zdhf_tjgz');
    const zdhf_bcgz = document.getElementById('zdhf_bcgz');
    const zdhf_sxgz = document.getElementById('zdhf_sxgz');
    (async function() {
        await zdhf_loadRules();
    })();
    zdhf_tjgz.addEventListener('click', () => { 
        // 构造新规则
        const newRule = {
            gjc: { "新关键词": "新回复" },
            sfjt: "false",
        ...
        // 添加到数据数组末尾
        zdhf_data.push(newRule);
        // 选中新添加的规则（最后一条）
        zdhf_xzxm = zdhf_data.length - 1;
        // 刷新界面
        zdhf_renderList();
        zdhf_updateReply();
        zdhf_showToast('已添加新规则', 'success');
    });
    zdhf_bcgz.addEventListener('click', () => { 
        (async function() {
            await zdhf_saveRules(zdhf_data);
        })();
    });
    zdhf_sxgz.addEventListener('click', () => { 
        //刷新
        (async function() {
            await zdhf_loadRules();
        })();
    });
    const container = document.getElementById('zdhf_lb');
    if (container) {
    ...
    const keywordInput = document.getElementById('zdhf_ppgjc');
    if (keywordInput) {
        keywordInput.addEventListener('input', function() {
            if (typeof zdhf_xzxm === 'undefined' || !Array.isArray(zdhf_data) || zdhf_data.length === 0) return;
            ...
            const checkbox = document.getElementById('zdhf_sfqy');
            if (checkbox) {
    ...
    // ---------- 删除规则 ----------
    const delRuleBtn = document.getElementById('zdhf_scgz');
    if (delRuleBtn) {
        delRuleBtn.addEventListener('click', function() {
            // 检查是否有选中的规则
            if (typeof zdhf_xzxm === 'undefined' || !Array.isArray(zdhf_data) || zdhf_data.length === 0) {
                zdhf_showToast('没有可删除的规则', 'error');
                return;
            }
    
            // 删除指定索引的规则
            zdhf_data.splice(zdhf_xzxm, 1);
    
            // 更新选中索引
            if (zdhf_data.length === 0) {
                zdhf_xzxm = undefined;  // 无数据时取消选中
            } else if (zdhf_xzxm >= zdhf_data.length) {
                zdhf_xzxm = zdhf_data.length - 1;  // 若删除的是最后一项，则选中新最后一项
            }
            // 如果 zdhf_xzxm 为 undefined，后续渲染会清空控件
    
            // 刷新界面
            zdhf_renderList();      // 重新渲染列表
            zdhf_updateReply();     // 更新输入框和多轮区域（内部会调用 zdhf_qtlcdh 清空或显示）
            zdhf_showToast('规则已删除', 'success');
        });
    }
    // ---------- 回复文本域 ----------
    const replyTextarea = document.getElementById('zdhf_hfnr');
    if (replyTextarea) {
    ...
    if (checkbox) {
        checkbox.addEventListener('change', function() {
            ...
            // 更新关键词输入框
            const input = document.getElementById('zdhf_ppgjc');
    ...
    // ---------- 多轮对话----------
    const dldhan = document.getElementById('zdhf_dldhan');
    if (dldhan) {
        dldhan.addEventListener('change', function() {
            ...
            // 列表不显示 sfjt，所以无需重绘列表；但如果你将来想在界面上显示状态，可调用 zdhf_renderList()
            zdhf_updateReply();
        });
    }
    // 监听多轮对话输入变化
    const dllb = document.getElementById('zdhf_dllb');
    if (dllb) {
        ...
        /**
         * 渲染规则列表到页面上
         * @param {Array} zdhf_data - 规则数组，每个元素包含 gjc, sfjt, jtc 等字段
         */
        function zdhf_renderList() {
            const container = document.getElementById('zdhf_lb');
            if (!container) {
                return;
            }
            ...
            // 如果规则为空或不是数组，显示提示
            if (!Array.isArray(zdhf_data) || zdhf_data.length === 0) {
                container.innerHTML = '<text class="lb-wtext">暂无规则</text>';
                return;
            }
        
            let html = '';
            zdhf_data.forEach((rule, idx) => {
                ...
                // 判断当前项是否激活（编号等于 zdhf_xzxm）
                const isActive = (typeof zdhf_xzxm !== 'undefined' && idx === zdhf_xzxm);
...
function zdhf_updateReply() {
    const keywordInput = document.getElementById('zdhf_ppgjc');
    const replyTextarea = document.getElementById('zdhf_hfnr');
    const checkbox = document.getElementById('zdhf_sfqy');
    const dldhan = document.getElementById('zdhf_dldhan');
    const dldhlb = document.getElementById('zdhf_dllb');
...
async function zdhf_loadRules() {
    try {
        ...
        if (result && result.success === true) {
            ...
            zdhf_showToast("加载失败: 返回格式错误", "error");
        }
        ...
        zdhf_renderList();
        zdhf_updateReply();
        zdhf_showToast(`加载成功，共 ${rules.length} 条规则`, "success");
    } catch (err) {
        console.error(err);
        zdhf_showToast("加载失败: " + err.message, "error");
    }
}
/**
 * 保存规则 JSON 到后端
 ...
 */
async function zdhf_saveRules(rules) {
    // ---------- 1. 数据校验 ----------
    if (!Array.isArray(rules) || rules.length === 0) {
        zdhf_showToast("没有可保存的规则", "error");
        return;
    }
    ...
    for (let i = 0; i < rules.length; i++) {
        ...
        // 基础校验：关键词和回复不能为空
        if (keys.length === 0) {
            zdhf_showToast(`第 ${i+1} 条规则缺少关键词`, "error");
            return;
        }
        const keyword = keys[0];
        const reply = gjc[keyword];
        if (!keyword || keyword.trim() === '') {
            zdhf_showToast(`第 ${i+1} 条规则的关键词不能为空`, "error");
            return;
        }
        if (!reply || reply.trim() === '') {
            zdhf_showToast(`第 ${i+1} 条规则的回复内容不能为空`, "error");
            return;
        }
        ...
        if (rule.sfjt === "true") {
            ...
            if (!jtc || typeof jtc !== 'object' || Array.isArray(jtc)) {
                zdhf_showToast(`第 ${i+1} 条规则（多轮模式）缺少 jtc 配置`, "error");
                return;
            }
            const roundKeys = Object.keys(jtc);
            if (roundKeys.length === 0) {
                zdhf_showToast(`第 ${i+1} 条规则（多轮模式）至少需要一个轮次`, "error");
                return;
            }
            for (let roundKey of roundKeys) {
                const options = jtc[roundKey];
                if (!Array.isArray(options) || options.length === 0) {
                    zdhf_showToast(`第 ${i+1} 条规则的第 ${roundKey} 轮没有选项`, "error");
                    return;
                }
                for (let optIdx = 0; optIdx < options.length; optIdx++) {
                    const opt = options[optIdx];
                    if (typeof opt !== 'object') {
                        zdhf_showToast(`第 ${i+1} 条规则的第 ${roundKey} 轮第 ${optIdx+1} 项格式错误`, "error");
                        return;
                    }
                    const optKeys = Object.keys(opt);
                    if (optKeys.length === 0) {
                        zdhf_showToast(`第 ${i+1} 条规则的第 ${roundKey} 轮第 ${optIdx+1} 项缺少关键词`, "error");
                        return;
                    }
                    const optKeyword = optKeys[0];
                    const optReply = opt[optKeyword];
                    if (!optKeyword || optKeyword.trim() === '') {
                        zdhf_showToast(`第 ${i+1} 条规则的第 ${roundKey} 轮第 ${optIdx+1} 项关键词不能为空`, "error");
                        return;
                    }
                    if (!optReply || optReply.trim() === '') {
                        zdhf_showToast(`第 ${i+1} 条规则的第 ${roundKey} 轮第 ${optIdx+1} 项回复不能为空`, "error");
                        return;
                    }
                }
            }
    ...
    try {
        if (!bridge) throw new Error("Bridge not available");
        await bridge.ready();
        const result = await bridge.apiPost("bcgz", { file: "zdhf.json", data: rules });

        // 判断保存是否成功
        const isSuccess = Array.isArray(result) || (result && result.success === true);
        console.log(isSuccess);
        if (isSuccess) {
            zdhf_showToast("保存成功", "success");
        } else {
            zdhf_showToast("保存失败: " + (result?.message || "未知错误"), "error");
        }
    } catch (err) {
        console.error(err);
        zdhf_showToast("加载失败: " + err.message, "error");
...
function zdhf_showToast(message, type = "success") {
